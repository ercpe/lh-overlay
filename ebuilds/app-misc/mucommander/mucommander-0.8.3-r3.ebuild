...
src_compile() {
	...
	if use excludable-mountpoints; then
		epatch ${FILESDIR}/01_excludablemountpoints.patch
	fi
...
pkg_postinst() {
	if use excludable-mountpoints; then
		einfo "To exclude mountpoints add a node like"
		einfo "   <excludeMountPoints>/usr/portage,/boot,/var/tmp/ccache</excludeMountPoints>"
		einfo "to your ~/.mucommander/preferences.xml"
	fi	
}